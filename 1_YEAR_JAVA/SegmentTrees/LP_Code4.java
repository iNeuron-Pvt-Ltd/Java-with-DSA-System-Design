...
class Main {
	...
	static void construct_segment_tree(int[] segtree, int[] a, int n) {
		for (int i = 0; i < n; i++) segtree[n + i] = a[i];
		for (int i = n - 1; i >= 1; i--) segtree[i] = Math.min(segtree[2 * i], segtree[2 * i + 1]);
	}
